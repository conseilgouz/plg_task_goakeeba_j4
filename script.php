<?php
/**
* Go Akeeba Plugin  - Joomla 4.x/5.x Plugin 
* Version			: 2.0.0
...
class plgtaskGoAkeebaInstallerScript
{
	private $min_joomla_version      = '4.1.0';
	private $min_php_version         = '8.0';
	private $name                    = 'Plugin Go Akeeba';
	private $exttype                 = 'plugin';
	private $extname                 = 'goakeeba';
	private $previous_version        = '';
	private $dir           = null;
	private $installerName = 'plgtaskgoakeebainstaller';
	...
	private function postinstall_cleanup() {
        ...
        $conditions = array(
            ...
            $db->qn('element') . ' = ' . $db->quote('goakeeba')
        );
        ...
        try {
            ...
            Log::add('unable to enable plugin go akeeba', Log::ERROR, 'jerror');
        }
		...
		$query = $db->getQuery(true)
			->delete('#__update_sites')
			->where($db->quoteName('location') . ' like "%conseilgouz.com/updates/%_goakeeba%"');
		$db->setQuery($query);
		$db->execute();
